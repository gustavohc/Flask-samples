...
from resources.userResources import UserRegister, UserResources, UserLogin, TokenRefresh, UserLogout
...
def miising_token_callback(error):
    return jsonify({
...
def token_not_fresh_callback():
    return jsonify({
        ...
        'error': 'token_fresh_required'
    }), 401

@jwt.revoked_token_loader
def revoked_token_callback():
    return jsonify({
        'description': 'The token has revoked.',
        'error': 'token_revoked'
    }), 401
...
api.add_resource(UserResources, '/user/<int:user_id>')
